...
[STRICT BLIND POLICY & INDEPENDENCE]
...
- 기존 Stage calibration dataset(46 snapshots, 27 unique tickers)과
  중복이 전혀 없는 100% 신규 24개 고유 ticker로 구성했다.
- snapshot_date 이후의 미래 결과나 수익률을 Stage 판정에 사용하는 lookahead 및
  outcome contamination을 철저히 배제했다.

[NON-PRICE NARRATIVE SEPARATION]
- Non-price narratives are descriptive context only.
- They were not used to determine manual_stage.
- `manual_stage_reason`과 `episode_notes`는 오직 가격 구조와 historical path에만 근거하며,
  업황/정책/테마/실적 등 비가격 배경은 `source_notes`에만 참고용 맥락으로 기재했다.
"""
...
PATTERN_A_STAGE_OOS_V01_LABELS: tuple[StageOOSSnapshotSpec, ...] = (
    ...
    StageOOSSnapshotSpec(
        ...
        "quiet_box_base",
        "3만원 초반대 바닥을 탄탄히 다지는 수평 횡보.",
        PatternAStage.BASE,
        "하방 경직성이 뚜렷하고 장기 이평선이 평탄화되어 있으며 아직 뚜렷한 상방 돌파 "
        "모멘텀은 없는 안정화 단계.",
        "장기 횡보 안정화 episode 형성 중.",
        "OOS v0.1 신규 선정 후보(경영진 선임 지연 이슈 등 대외 불확실성 속 바닥 다지기).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
    ...
    StageOOSSnapshotSpec(
        ...
        "quiet_box_base",
        "9만원대 초반에서 하방 경직성을 보이며 안정적 수평 횡보.",
        PatternAStage.BASE,
        "하락이 멈추고 9만원대에서 수개월간 좁은 박스를 형성하며 에너지를 모으는 BASE 단계.",
        "과거 하락 조정 마무리 후 신규 베이스 episode 진입.",
        "OOS v0.1 신규 선정 후보(레고켐바이오 인수 이슈 이후 주가 안정화 맥락).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
        "068270", "셀트리온", "2023-09-30",
        "cycle_reset_base",
        "2020~2021년 35만원대에서 14만원대까지 2년간 하락 후 1년 이상 바닥 다지기.",
        PatternAStage.BASE,
    ...
    StageOOSSnapshotSpec(
        ...
        "OOS v0.1 신규 선정 후보(반도체 다운사이클 바닥 통과 맥락).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
    ...
    StageOOSSnapshotSpec(
        ...
        "OOS v0.1 신규 선정 후보(수출 모멘텀 태동 맥락).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
    ...
    StageOOSSnapshotSpec(
        ...
        PatternAStage.TRANSITION,
        "월봉 장기선은 아직 평탄하나 주봉이 강력하게 상방으로 꺾이며 박스 상단을 노크하는 "
        "전형적 Weekly leads TRANSITION.",
        "금융지주 장기 박스권 상향 돌파 시도 episode.",
        "OOS v0.1 신규 선정 후보(당시 기업 밸류업 프로그램 관련 시장 관심 맥락).",
        "HIGH",
    ),
    ...
    StageOOSSnapshotSpec(
        ...
        "전환기를 거쳐 대세 상승의 1차 확장이 막 전개되는 초기 추세 국면.",
        "OOS v0.1 신규 선정 후보(HBM 등 AI 반도체 수요 모멘텀 본격화 맥락).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
    ...
    StageOOSSnapshotSpec(
        ...
        "우상향을 형성하는 초기 추세 단계.",
        "대세 상승 episode의 본격 전개 국면.",
        "OOS v0.1 신규 선정 후보(전력 인프라/해저케이블 수요 확대 맥락).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
        ...
        "OOS v0.1 신규 선정 후보(해외 수주 회복 사이클 맥락).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
        "003230", "삼양식품", "2022-11-30",
        "clean_early_trend",
        "11.1만원에 안착하며 2년여의 박스 상단을 돌파하고 우상향 개시.",
        PatternAStage.EARLY_TREND,
        "장기 저항선을 확실히 넘어서고 주봉/월봉 기울기가 모두 탄탄한 양수를 유지하는 "
        "정석적 EARLY_TREND.",
        "대세 상승 episode 1차 도약 국면.",
        "OOS v0.1 신규 선정 후보(불닭볶음면 글로벌 수출 고성장 맥락).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
        "272210", "한화시스템", "2024-03-31",
        "clean_early_trend",
        "1.7만원대 장기 저항선을 상방 돌파하며 52주 신고가 갱신.",
        PatternAStage.EARLY_TREND,
        "장기 박스 상단을 돌파하여 52주 신고가를 갱신하고 이평선이 상향 발산하기 시작한 "
        "초기 추세 단계.",
        "신규 상승 추세 episode 진입.",
        "OOS v0.1 신규 선정 후보(K-방산 수출 호조 맥락).",
        "HIGH",
    ),
    ...
    StageOOSSnapshotSpec(
        ...
        "돌파 기준점(10~13만원) 대비 2배 이상 상승하고 36개월 레인지 96% 위치에 도달하여 "
    ...
    StageOOSSnapshotSpec(
        ...
        PatternAStage.PROGRESSED,
        "초기 돌파가 이미 1년 반 전에 끝나고 주가가 가파르게 상승하여 이평선 간격과 가격이 "
    ...
    StageOOSSnapshotSpec(
        ...
        PatternAStage.PROGRESSED,
        "역사상 유례없는 숏스퀴즈 및 수직 상승으로 가격이 통상적 밴드를 완전히 이탈하여 "
        "극단적 과열 확장에 도달한 PROGRESSED.",
        "상승 episode의 최종 버블 확장 단계.",
        "OOS v0.1 신규 선정 후보(2차전지 광풍 관련 시장 맥락).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
    ...
    StageOOSSnapshotSpec(
        ...
        "2020년 하반기 시작된 대세 상승 episode의 후반 확장 국면.",
    ...
    StageOOSSnapshotSpec(
        ...
        "넓게 확장된 성숙 추세 국면.",
        "지주사 통합 이후 전개된 대세 상승 episode의 확장 국면.",
        "OOS v0.1 신규 선정 후보(주주환원 정책 관련 시장 맥락 존재).",
        "HIGH",
    ),
    StageOOSSnapshotSpec(
        "006260", "LS", "2023-07-31",
        "extended_progressed",
        "12만원 돌파 급등, 2023년 초 6만원대 돌파 이후 2배 이상 단기 폭등한 과열 확장.",
        PatternAStage.PROGRESSED,
