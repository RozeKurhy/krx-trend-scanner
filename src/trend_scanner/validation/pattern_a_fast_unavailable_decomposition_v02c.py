"""FAST + Pattern A UNAVAILABLE Decomposition Validation v0.2C Module.

Evaluates:
  Primary Question 1:
    "Why was Pattern A UNAVAILABLE for 473 cases at first FAST trigger?"
    (Decomposed into INSUFFICIENT_PATTERN_A_HISTORY, PATTERN_A_FEATURE_UNAVAILABLE,
     PATTERN_A_EVALUATION_EXCEPTION, PATTERN_A_STAGE_MISSING, OTHER_UNAVAILABLE)
  Primary Question 2:
    "How do FAST_UNAVAILABLE forward outcomes (4W, 8W, 12W, 26W Return, MFE, MAE, Tails)
     compare against FAST_TRANSITION and FAST_WEAK?"
  Primary Question 3:
    "When UNAVAILABLE resolves post-entry, what is the First Valid Pattern A Stage
     (WEAK, BASE, TRANSITION, EARLY_TREND, PROGRESSED, NEVER_AVAILABLE) and Lead Time?"
  Secondary Diagnostics:
    - Post-entry lifecycle follow-through (ever WEAK, BASE, TRANSITION, EARLY_TREND, PROGRESSED).
    - Available History Length buckets (<12m, 12-<24m, 24-<36m, >=36m).
    - Era, Market, Risk Grade, and FAST Score diagnostics.
...
Strict Invariants:
  ...
  - PRODUCTION_HOLD (zero production impact).
"""
...
@dataclass
class FastUnavailableSignalRecord:
    ticker: str
    name: str
    market: str
    ...
    unavailable_reason_primary: str
    unavailable_reason_detail: str

    available_daily_days: int
    available_weekly_bars: int
    available_monthly_bars: int
    ...
    first_valid_pa_stage_date: str | None
    first_valid_pa_stage: str
    days_to_first_valid_pa_stage: int | None

    ever_weak: bool
    first_weak_date: str | None
    days_to_weak: int | None

    ever_base: bool
    first_base_date: str | None
    days_to_base: int | None
...
@dataclass
class TickerUnavailableDiagnostic:
    ticker: str
    name: str
    market: str
    evaluation_status: str
    ...
    unavailable_reason_primary: str | None
    warning_count: int
    first_exception_type: str | None
    first_exception_message: str | None


def simulate_ticker_unavailable_decomposition(
    ticker: str,
    name: str,
    market: str,
...
) -> tuple[TickerUnavailableDiagnostic, FastUnavailableSignalRecord | None]:
    """Simulate FAST first signal and decompose UNAVAILABLE causes + post-entry valid stages."""
    if daily is None or daily.empty:
        diag = TickerUnavailableDiagnostic(
            ticker=ticker,
            name=name,
            market=market,
            ...
            unavailable_reason_primary=None,
            warning_count=0,
    ...
    # Research Cohort
    if fast_first_pa_stage == "UNAVAILABLE":
    ...
    # Build snapshot at signal date to inspect history and unavailable cause
    sliced_daily = daily[daily.index <= fast_first_w]
    avail_daily_days = len(sliced_daily)
    avail_history_days = (fast_first_w - sliced_daily.index.min()).days if not sliced_daily.empty else 0

    snap_at_signal = build_historical_snapshot(ticker, name, sliced_daily, fast_first_w, include_incomplete_periods=False)
    avail_weekly_bars = len(snap_at_signal.weekly)
    avail_monthly_bars = len(snap_at_signal.monthly)

    # Decompose UNAVAILABLE cause
    if fast_first_pa_stage != "UNAVAILABLE":
        unav_primary = "NOT_UNAVAILABLE"
        unav_detail = f"valid_stage_{fast_first_pa_stage}"
    else:
        # Check why it was unavailable
        if avail_monthly_bars < 36:
            unav_primary = "INSUFFICIENT_PATTERN_A_HISTORY"
            unav_detail = f"monthly_bars_{avail_monthly_bars}_lt_36"
        else:
            # Check missing features
            missing_feats = [
                f for f in _REQUIRED_STAGE_FIELDS
                if pd.isna(getattr(snap_at_signal.features, f, np.nan))
            ]
            if missing_feats:
                unav_primary = "PATTERN_A_FEATURE_UNAVAILABLE"
                unav_detail = f"missing_{'_'.join(missing_feats)}"
            else:
                unav_primary = "PATTERN_A_STAGE_MISSING"
                unav_detail = "features_present_but_stage_none"
    ...
    diag = TickerUnavailableDiagnostic(
        ...
        non_executable_reason=None,
        research_cohort=research_cohort,
        unavailable_reason_primary=unav_primary,
        warning_count=warning_count,
        first_exception_type=first_ex_type,
        first_exception_message=first_ex_msg,
    )
    ...
    first_valid_stage_d: pd.Timestamp | None = None
    first_valid_stage_val: str = "NEVER_AVAILABLE"

    ever_weak = False
    first_weak_d: pd.Timestamp | None = None
    ever_base = False
    first_base_d: pd.Timestamp | None = None
    ever_trans = False
    ...
    for m in m_dates:
        try:
            ...
            if st != "UNAVAILABLE" and first_valid_stage_d is None:
                first_valid_stage_d = m
                first_valid_stage_val = st

            if st == "WEAK" and not ever_weak:
                ever_weak = True
                first_weak_d = m
            if st == "BASE" and not ever_base:
                ever_base = True
                first_base_d = m
    ...
    days_valid = (first_valid_stage_d - fast_first_w).days if first_valid_stage_d else None
    days_weak = (first_weak_d - fast_first_w).days if first_weak_d else None
    days_base = (first_base_d - fast_first_w).days if first_base_d else None
    ...
    # Update diagnostic warning provenance with post-entry execution warnings
    diag.warning_count = warning_count
    diag.first_exception_type = first_ex_type
    diag.first_exception_message = first_ex_msg
    ...
    record = FastUnavailableSignalRecord(
        ticker=ticker,
        name=name,
        market=market,
        ...
        research_cohort=research_cohort,
        unavailable_reason_primary=unav_primary,
        unavailable_reason_detail=unav_detail,
        available_daily_days=avail_daily_days,
        available_weekly_bars=avail_weekly_bars,
        available_monthly_bars=avail_monthly_bars,
        available_history_days=avail_history_days,
        ...
        first_valid_pa_stage_date=first_valid_stage_d.strftime("%Y-%m-%d") if first_valid_stage_d else None,
        first_valid_pa_stage=first_valid_stage_val,
        days_to_first_valid_pa_stage=days_valid,
        ever_weak=ever_weak,
        first_weak_date=first_weak_d.strftime("%Y-%m-%d") if first_weak_d else None,
        days_to_weak=days_weak,
        ever_base=ever_base,
        first_base_date=first_base_d.strftime("%Y-%m-%d") if first_base_d else None,
        days_to_base=days_base,
