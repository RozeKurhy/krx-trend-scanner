...
# 수정주가(open/high/low/close)와 거래대금을 서로 다른 두 API 호출로 가져와
# 날짜 기준으로 합치기 때문에, 두 응답의 거래일이 완전히 일치하지 않으면
# trading_value에 NaN이 생길 수 있다. OHLC는 항상 단일 조회 결과이므로 NaN을
# 허용하지 않는다.
...
def validate_ohlcv(df: pd.DataFrame) -> None:
    ...
    if null_rows.any():
        raise MarketDataError(f"OHLC에 NaN이 있습니다: {list(df.index[null_rows])}")
