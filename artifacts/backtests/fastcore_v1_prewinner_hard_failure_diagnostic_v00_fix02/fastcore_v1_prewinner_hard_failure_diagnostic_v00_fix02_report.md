# FASTCORE V1 PRE-WINNER HARD FAILURE DIAGNOSTIC V00 FIX02

## FIX02 핵심 질문에 대한 숫자 답

### Q1. SIGNAL_CUTOFF → SUPPORT_END horizon 수정으로 분류가 몇 건 바뀌었는가?

- RECOVERY: 737 → 737건 (delta +0), NEVER_WINNER: 236 → 236건 (delta +0)야. Loss Guard는 RECOVERY +0, NEVER_WINNER +0건 변했어.

### Q2. threshold sweep 결과가 얼마나 변했는가?

- 2/10개 threshold 행에서 변화가 있었어: -55%, -60%. 전체 threshold별 FIX01 → FIX02 수치는 아래 비교표와 CSV에 기록했어.

### Q3. 특히 -30%는 어떻게 변했는가?

- recovery kill rate: 16.010855% → 16.010855% (delta 0.000000%), failure capture rate: 75.847458% → 75.847458% (delta 0.000000%), positive rate: 60.226105% → 60.226105% (delta 0.000000%), mean return: 5.261881% → 5.261881% (delta 0.000000%)야.

### Q4. horizon correction 이후에도 fixed Hard Failure threshold를 식별할 수 없는가?

- **예**. `hard_failure_threshold_identified=false`, candidate는 `None`로 유지해.

### Q5. `fixed_threshold_mean_improvement_count`는 몇 개인가?

- FIX01 0개 → FIX02 0개 (delta +0)야.

### Q6. `-25~-30%`를 FAILURE ARMED 연구 참고 영역으로 유지할 수 있는가?

- **유지할 수 있어**. 이번 horizon correction에서도 해당 구간을 Hard Exit나 strategy parameter로 확정하지 않고, 향후 FAILURE ARMED price-damage 연구 참고 영역으로만 유지해.
...
- RECOVERY: 737 → 737 (delta +0)
- NEVER_WINNER: 236 → 236 (delta +0)
- Loss Guard RECOVERY: 387 → 387 (delta +0)
- Loss Guard NEVER_WINNER: 203 → 203 (delta +0)
- -30% RECOVERY kill rate: 16.010855% → 16.010855% (delta 0.000000%)
- -30% failure capture rate: 75.847458% → 75.847458% (delta 0.000000%)
- -30% mean return: 5.261881% → 5.261881% (delta 0.000000%)
- fixed threshold mean improvement count: 0 → 0 (delta +0)
...
| -55% | 20 → 20 | 86 → 87 | 69.270298% → 69.270298% | 7.678407% → 7.680606% | 8.840000% → 8.840000% | -0.002055d |
| -60% | 13 → 13 | 62 → 63 | 69.886948% → 69.886948% | 8.050545% → 8.050946% | 9.070000% → 9.070000% | 0.000000d |
...
상세 산출물 절대 경로: `/Users/june/Documents/projects/krx-trend-scanner/artifacts/backtests/fastcore_v1_prewinner_hard_failure_diagnostic_v00_fix02`
