...
from trend_scanner.data.krx_quota_reconciliation import file_sha256, reconcile_known_attempts
...
def _payload(market: str = "KOSPI", *, ticker: str = "005930") -> dict:
    return {
        "OutBlock_1": [{
            "ISU_CD": "KR7005930003",
            "ISU_SRT_CD": ticker,
...
def _client(tmp_path: Path, opener, *, reserve: int = 500, endpoint_limit: int = 1000, global_limit: int = 1000):
    ...
    client = KrxOpenApiClient("test-key", opener=opener, max_transient_retries=1, sleeper=lambda _seconds: None, quota=quota)
    return client, quota
...
def test_live_reserve_must_be_500_and_direct_bypass_is_rejected(tmp_path):
    ...
    with pytest.raises(ValueError, match="canonical quota"):
        HistoricalInstrumentAcquisitionRunner(bypass, LocalKrxOpenApiQuota(tmp_path / "bypass.sqlite3", reserve=500)).run(["2026-08-21"], execute_live=True)
...
def test_live_success_writes_atomic_raw_and_checkpoint(tmp_path):
    ...
    summary = runner.run(["2026-08-21"], execute_live=True)
...
def test_resume_skips_verified_complete_and_repairs_broken_complete(tmp_path):
    ...
    summary = HistoricalInstrumentAcquisitionRunner(client, quota, raw_root=tmp_path / "raw", checkpoint_path=tmp_path / "manifest.json").run(["2026-08-21"], execute_live=True)
    assert summary["network_attempts"] == 0
    ...
    repaired = HistoricalInstrumentAcquisitionRunner(client, quota, raw_root=tmp_path / "raw", checkpoint_path=manifest_path).run(["2026-08-21"], execute_live=True)
