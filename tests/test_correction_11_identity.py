"""CORRECTION_11 runner isolation, identity completeness, and binding tests."""
...
def _validate(bundle: dict):
    return ca.validate_live_evidence_linkage(
        ...
        accounting_cross_invariant_pass=True, schema_suffix="11",
    )
...
def test_correction11_offline_guard_isolated_paths(tmp_path, monkeypatch):
    monkeypatch.setenv("CORRECTION_11_OFFLINE_ONLY", "1")
    result = ca.run_corporate_action_evidence_acquisition_fix03_correction_11(output_dir=tmp_path / "v01_fix03_correction_11")
    assert result["recommended_next_state"].endswith("CORRECTION_11")
    names = {p.name for p in (tmp_path / "v01_fix03_correction_11").iterdir()}
    assert names
...
def test_correction11_binding_has_no_future_head(monkeypatch):
    ...
    binding = {
        "schema": "code_test_binding_evidence_v01_fix03_correction_11",
...
def test_correction11_mocked_full_production_success(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CORRECTION_11_OFFLINE_ONLY", raising=False)
    ...
    result = ca.run_corporate_action_evidence_acquisition_fix03_correction_11(
        output_dir=tmp_path / "v01_fix03_correction_11", allow_network=True
    )
    ...
    files = {p.name for p in (tmp_path / "v01_fix03_correction_11").iterdir()}
    assert files
    assert not any("correction_9" in name or "correction_10" in name for name in files)
    assert result["directive_id"].endswith("CORRECTION_11")
