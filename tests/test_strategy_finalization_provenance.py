...
def test_provenance_consistency_across_artifacts():
    ...
    # 2. Canonical Corrected PIT Evaluation JSON
    canonical_eval_path = ROOT / "artifacts/patterns/pattern_a_fast/production/strategy_finalization_v01/pattern_a_fast_strategy_finalization_v01_evaluation.json"
    canonical_data = json.loads(canonical_eval_path.read_text(encoding="utf-8"))
    c_meta = canonical_data["metadata"]
    assert c_meta["evaluation_basis"] == "CORRECTED_PIT_BASELINE"
    assert c_meta["calendar_authority_commit"] == "88d54d85bdee1f2121bec9b27a250cbc1cb9f98f"
    assert c_meta["corrected_evaluation_commit"] == "f73e0c23b10cc3e3f8215693ef5095b2c0f6716d"
    assert c_meta["total_common_universe"] == 2528
    assert c_meta["phase10_investable_universe"] == 1081
    assert c_meta["primary_trade_count"] == 551
    assert c_meta["transition_count"] == 477
    assert c_meta["early_trend_count"] == 74

    # 3. Legacy Archived Evaluation JSON
    legacy_eval_path = ROOT / "artifacts/patterns/pattern_a_fast/archive/strategy_finalization_v01_legacy/pattern_a_fast_strategy_finalization_v01_evaluation.json"
    legacy_data = json.loads(legacy_eval_path.read_text(encoding="utf-8"))
    l_meta = legacy_data["metadata"]
    assert l_meta["architecture_authority_commit"] == "89df82a938dba1961c2342064db2dc0061a5f2ca"
    assert l_meta["preregistration_commit"] == "a5c29e7e97cb7e6830c3dcd25d824e5779f2312f"
