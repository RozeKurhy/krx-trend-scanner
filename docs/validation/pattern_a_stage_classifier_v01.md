...
`pattern_a_stage_manifest.py`의 46건 manual truth set을 근거로, Score와
독립적이고 rule-based로 설명 가능한 첫 production candidate Stage
classifier(`src/trend_scanner/patterns/pattern_a_stage.py`)를 구현했다.
...
StageLifecycleContext  # 현재 episode 안에서의 과거 확장/붕괴 여부
...
전부 46건 truth set에 기록된 실제 Feature 값을 손으로 대조해 잡았다(ML
fitting이 아니다). 세 가지는 처음 설계에서 실측으로 틀렸다는 게 확인돼
수정한 것이다.
...
* `previously_expanded_in_current_episode`: 과거 어느 달에
  `ma24_slope>0 and avg_price_change_12m>=0.30`(PROGRESSED 직접 판정과
  동일 기준)이 성립한 적이 있는가.
* `episode_broken`: 그 확장 시점 이후~현재 이전 구간에
  `ma24_slope<=-0.045` 또는 `range_position<=0.20`이 한 번이라도
  있었는가.
...
"마지막 확장"으로 잡히는 문제였다. 그래서 최종 기준은 PROGRESSED 직접
판정과 동일한 `ma24_slope>0 AND avg_price_change_12m>=0.30`(AND, 훨씬
엄격함)으로 좁혔다.
...
"이미 이 episode 안에서 확장했었고 아직 안 꺾였으면(`episode_broken=False`)
PROGRESSED로 유지한다"는 override를 넣어서 46건에 실측 검증했다. 이
override는 LIG넥스원(079550) 2023-12-31 같은 사례(지금 이 순간의
evidence는 `breakout_like_structure`까지만 도달하지만, 실제로는
PROGRESSED가 맞는 lifecycle continuation)를 잡아주기 위해 설계했다.
...
잘못 PROGRESSED로 밀어올리는 부작용이 더 컸다. `episode_broken` 판정
(가파른 하락이나 낮은 range_position)이 "장기 횡보로 국면이 자연스럽게
끝난" 경우를 못 잡기 때문이다.
...
| ticker | name | snapshot_date | predicted_stage | 관찰 |
|---|---|---|---|---|
...
**Pattern A Stage Classifier v0.1 — Accepted as v0.1 baseline.**

Score와 독립적이고, 8개 evidence + episode/cycle reset context 전부를
사람이 reason_codes로 그대로 따라 읽을 수 있는 rule-based classifier를
구현했다. 46건 truth set에서 exact match 82.6%(38/46), SEVERE mismatch
3건(6.5%)이며, 3건의 challenge case에서도 급격히 튀지 않는 정성적으로
말이 되는 예측을 냈다. 알려진 실패 모드 4가지(episode continuation,
...
했지만, 전체 정확도가 떨어져서 최종적으로 폐기했다). v0.2는 이 실패
모드들을 개별적으로 좁히는 방향으로 진행한다.
