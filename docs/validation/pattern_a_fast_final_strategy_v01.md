...
1. **손실 가드 규칙 (`HOLD_B_PRE_PROGRESSED_LOSS_GUARD`)**:
   ...
   - **활성 구간 (`active_window`)**: `AFTER_ENTRY_EXECUTION AND BEFORE_FIRST_PROGRESSED_SNAPSHOT` (`CURRENT_DATE_LT_FIRST_PROGRESSED_SNAPSHOT_DATE`)
   - **체결 타이밍**: 신호 발생 익영업일 시가 (**`NEXT_LOCAL_TRADING_DAY_OPEN`**)
   - **청산 식별자**: `LOSS_GUARD_CLOSE_LE_NEG_15`
2. **PROGRESSED 진입 시 전환 규칙**:
   - 최초 `PROGRESSED` 월봉 스냅샷 당일부터 Loss Guard는 **비활성화(`INACTIVE`)**되며, PROGRESSED 청산 아키텍처 권한으로 전환됨.
3. **금지 규칙**:
   - Time Stop: `NONE`
