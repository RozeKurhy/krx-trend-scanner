...
- TTM = 최신 4개 연속 독립 분기의 합
- 분기 YoY = 현재 분기와 4분기 전 동일 분기 비교
- 연간 YoY = 현재 FY와 직전 FY 비교
- 영업이익률 = operating income / revenue * 100
- 순이익률 = net income / revenue * 100
...
- 확인되지 않은 기간, 재무제표 기준/통화 불일치, 모호한 기간·공시 문맥,
  미래 공시는 값을 0으로 대체하지 않고 그대로 미확인 상태로 남긴다.
...
- Pattern A + Fundamentals 합산 점수 등 모든 합산 점수
- 자동 매매 신호/추천
- 금융회사 특화 재무 지표 (NIM, CET1 등)
- 배당 분석
- DCF, 적정가치, 목표가
- PyKRX, KRX HTML/웹 수집, Naver 원천 데이터 대체 경로를 정본 원천으로
  사용
- 기존 ETF/parquet 데이터를 OpenDART 정본 원천 데이터로 승격
