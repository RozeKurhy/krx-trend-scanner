...
| 조건 | 기준 |
|---|---|
| 연간 매출 | `MultiPeriodFundamentalsResult`의 `latest_fy`가 가리키는 최신 V1 사용 가능 FY revenue >= 50,000,000,000 KRW |
| 최근 분기 평균 매출 | `latest_quarter`가 끝점인 최신 4개 연속 독립 분기 revenue 평균 >= 10,000,000,000 KRW |
...
(strict boundary), 연속 분기 누락/최신 분기 누락, 기준 기간 불일치
(endpoint mismatch), as-of 불일치, 비 `READY` 상태 보존, 금융업
`NOT_APPLICABLE`, 사용자 설정 및 JSON 직렬화를 고정한다.
