...
실제 데이터 제공자/store 기본 요소로 구현한 계약이다. 최종 상태는
...
- Parquet 물리 schema와 metadata sidecar, SHA-256 pair integrity를 보존한다.
...
왕복 저장하고 date/row/OHLC parity를 비교한다. 이는 `STORE_ROUND_TRIP` 근거다.
