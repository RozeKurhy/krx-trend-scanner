...
`KEEP + ARCHIVE + DELETE_CANDIDATE = 19 + 3 + 0 = 22`로 원본 조사 대상 수와
일치한다.
...
| `docs/architecture/krx_index_series_mapping_v01.md` | Native sector index validation 계약 | KEEP | native 46개 sector index의 identity 검증과 production Sector RS 미교체 경계를 정의하는 현재 validation contract다. | 유지. validation-only와 production 전환을 구분한다. |
| `docs/architecture/krx_open_api_v02_validation.md` | KRX Open API V02 bounded validation 경계 | KEEP | endpoint semantic, PIT snapshot, quota 40회 경계와 production migration 금지를 정의하는 현재 검증 경계다. | 유지. live evidence 없이는 migration을 선언하지 않는다. |
...
| `docs/architecture/validation/historical_market_cap_backfill_v01.md` | Historical market-cap PIT source correction 계약 | KEEP | 22개 active reference grid와 보정된 KRX canonical source를 다음 Phase 13J-1이 소비하도록 고정하고 `READY_FOR_ADVISOR...REVIEW` 상태를 명시한다. 역사 날짜를 다루지만 현재 active reference다. | 유지. 다음 phase가 소비할 frozen source 기준으로 사용한다. |
...
- `krx_open_api_v02_validation.md`: bounded validation contract와 실행 경계를
  정의하지만 live approval 여부를 스스로 확정하지 않으므로 KEEP하되 migration
  authority로 해석하지 않는다.
- `historical_market_cap_backfill_v01.md`: 과거 날짜 source를 다루지만 다음
  phase가 사용할 active frozen reference이므로 ARCHIVE하지 않았다.
