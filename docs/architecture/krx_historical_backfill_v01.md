...
이번 phase의 자연 단위는 `market + basDd` 전체 시장 스냅샷이며,
...
`ISU_CD`는 이번 일별 endpoint에서 이미 6자리 ticker 의미이므로 그대로 엄격히
...
후보 날짜는 `pd.bdate_range()`의 평일 스케줄러일 뿐 KRX trading calendar가
...
`resume`은 integrity-valid COMPLETE/NO_DATA를 API 호출 없이 건너뛴다. FAILED는
...
V01 대상은 2010-01-04부터 2026-08-21까지다. 완료 게이트는 각 weekday candidate가
