...
웹은 원천 데이터를 직접 계산하거나 프로젝트의 권위 기준을 다시 정의하는
계층이 아니다. 프로젝트에서 이미 생성·검증된 원천 데이터와 분석 결과를
웹에서 읽기 쉬운 정적 JSON으로 변환해 전달한다.
...
| 외인 순매수 | 외인 수급 원천, 보통주 종목 집합·섹터 구성, Repository V2 가격 조회, 공개 리포트 범위 | [`export_foreign_net_buy_ranking_web.py`](../../scripts/export_foreign_net_buy_ranking_web.py) | [`foreign-net-buy-ranking.json`](../../web/data/foreign-net-buy-ranking.json) | [`foreign.js`](../../web/js/foreign.js) | [`foreign.html`](../../web/foreign.html) |
...
- 마켓 RS와 외인 순매수는 공개 리포트 범위와 종목 식별 정보를 확인한 뒤 각자
  랭킹 JSON을 만든다.
...
- 현재 구현에서 다른 웹 투영 결과를 입력으로 사용하는 경우는 숨기지 않는다.
  마켓·외인 랭킹의 공개 리포트 범위와 전략 운용 모니터가 이에 해당한다.
