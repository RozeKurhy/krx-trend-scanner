# Pattern B 사람 판정 표본 원시 지표값 V01

> 상태: 36개 표본의 원시 지표값 계산·봉인 완료. 사람 판정과 지표의 적합성
> 분석은 아직 하지 않았다. 임계값, 점수, 자동 상태 판정도 없다.

## 목적

[사람 판정 V01](human_ground_truth_v01.md) 표본 36개에 대해
[지표 계약 V01](../spec/feature_contract_v01.md)의 7개 지표 원시값을 계산해
`sample_id` 기준으로 고정한다. 다음 단계의 적합성 분석은 이 값을 그대로 쓴다.

- 사람 판정 결과는 지표 계산 전에 이미 봉인했다.
- 지표 산식과 PIT 계약은 이 계산을 위해 바꾸지 않았다.
- 비공개 대응표는 `sample_id`를 종목코드·기준일에 연결하는 데만 썼다. 대응표와
  종목코드·종목명·기준일은 Git에 올리지 않았다.
...
(36행, `sample_id`와 7개 지표의 값·상태만 담은 15개 열)
