...
- 기준일에 거래정지 중인 종목은 마지막 거래 봉으로 상태가 계산된다.
  [전체 종목 운영 감사 V01](../validation/full_universe_operational_audit_v01.md)(2026-09-21)에서
  89종목이 이 경우였다.
- 전체 종목 운영 감사 V01에서 가격 로딩·종목 구간 적용·평가를 2,544종목에 한 번 점검했다(`PASS`).
  계산 불가는 10.6%였다. 이 감사는 한 기준일의 결과이며, 일일 갱신 운영을 검증한 것은 아니다.
- 시장 이전(예: KOSDAQ → KOSPI) 뒤에는 종목 구간이 새로 시작되어 이전 가격 이력을 쓰지 않는다.
