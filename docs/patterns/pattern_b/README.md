...
상태 판정 규칙 V01은 별도 검증 표본 V02로 공식 1회 평가를 마쳤다. 채택 여부는 아직 정하지
않았으며, 점수와 전략은 없다.
...
| 현재 상태 | 초기 연구 후보 (다음 단계: 평가 결과에 따른 연구 판단) |
| 시간축 | 월봉(핵심), 주봉(보조) |
| 사람 판정 | V01 36개 판정 완료·봉인. 별도 검증 표본 V02 사람 판정 완료·봉인 |
| 지표 | V01 산식·PIT 계약 확정, 적합성 진단 V01 완료, [선택 V01](validation/feature_selection_v01.md) 결정 완료 (유지 3개) |
| 상태 판정 규칙 | [V01](validation/state_rule_v01.md) 설계·봉인 완료. [별도 검증 표본 V02 공식 1회 평가](validation/state_rule_v01_holdout_v02_evaluation.md) 완료, V02는 소진됨 |
| 전략·백테스트 | 없음 |
...
- [개발 표본 V02](validation/development_v02_protocol.md) — 사람 판정 기준 V02용 개발 표본 36개.
  [사람 판정](validation/development_v02_human_labels.md) 완료·봉인, 지표 계산 전
