...
Score v0.2와 Stage v0.1은 현재 공식 계약이다. 과거 문서에 남은 `NEXT`,
`PENDING`, OOS2 미실행 등의 표현은 문서 작성 당시의 상태이며 현재 작업
상태가 아니다. 추가 검증이나 규칙 변경이 필요하면 별도 연구와 승인 절차로
다룬다.
...
Stage는 Score 결과를 이용해 분류하며 Stage가 Score를 역으로 수정하지 않는다.

| 순서 | 조건 | Stage |
|---:|---|---|
| 1 | `base_score` 또는 `transition_score`가 결측 | `None` |
| 2 | `progressed_evidence_count >= 3` | `PROGRESSED` |
| 3 | `transition_score < 40` 및 `base_score >= 60` | `BASE` |
| 4 | `transition_score < 40` 및 `base_score < 60` | `WEAK` |
| 5 | `transition_score >= 70` | `EARLY_TREND` |
| 6 | 그 밖의 경우 | `TRANSITION` |

Stage 분류는 현재 코드의 v0.1 기준이다. 역사적 검증에서 확인된 heuristic
한계는 과거 검증 기록으로 보존하며, 이 문서에서 임의로 threshold를 조정하지
않는다.
...
현재 계약의 근거는 다음 역사 기록에서 확인할 수 있다. 이 문서의 현재 규칙을
대체하지 않으며, 기록 당시의 `NEXT`, `HOLD`, OOS 대기 표현은 현재 상태로
읽지 않는다.
...
<details>
<summary>과거 연구·검증 기록 원문</summary>
