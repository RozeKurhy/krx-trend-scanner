...
> 상태 전이 원칙을 정의해. 주봉 Stage는 자동 매수 신호가 아니며, Feature·Threshold·
> Score를 계산하는 규칙은 이 문서의 범위에 포함하지 않아.
...
Pattern A FAST의 공식 주봉 Stage는 다음 다섯 가지야.
...
개념적 진행 순서는 `WATCH < SETUP < TRIGGER < TREND < EXTENDED`야. 하지만 이
표현은 좋음·나쁨이나 Score의 순위가 아니라 생애주기 위치를 뜻해.

Stage는 현재 PIT 시점의 주봉 구조를 표현해. 과거에 어느 Stage까지 갔는지나
이전 주의 Stage만으로 현재 Stage를 결정하지 않아. 따라서 생애주기는 비가역적
State Machine이 아니고, 후퇴와 직접 전이를 허용해.

## 시간축 역할

Pattern A FAST는 서로 다른 시간축의 책임을 섞지 않아.

- `monthly_regime`: 장기 환경과 국면
- `weekly_lifecycle_stage`: 현재 주봉 구조와 생애주기 위치
- `daily_timing`: 주봉 구조가 유효하다는 전제의 진입 타이밍 보조

이번 계약에서 동결하는 것은 `weekly_lifecycle_stage`의 의미뿐이야. 월봉과 일봉의
분류 체계·계산은 주봉 Stage의 의미에 섞지 않아.

예를 들어 `Monthly: BAD`, `Weekly: TRIGGER`, `Daily: READY` 같은 조합은 기술적으로
가능하고 오류가 아니야. 최종 매매 후보인지 여부는 별도 전략 계약에서 판단해.

## `WATCH`

핵심 질문은 “주봉 기준으로 아직 상승 전환 `SETUP`이 충분히 형성되지 않았는가?”야.

주봉 구조에서 상승 전환을 연구할 이유는 있을 수 있지만, `SETUP`이라고 부를 만큼
구조적 개선 증거가 충분하지 않은 초기 관찰 상태를 뜻해.
...
미약함, Range 안에서 의미 있는 구조 변화가 부족함이야. 구체 Feature는 별도
연구에서 정해.

## `SETUP`

핵심 질문은 “상승 전환 가능성을 보여주는 주봉 구조 변화가 실제로 시작됐는가?”야.

단순한 가격 반등을 넘어 주봉 구조에서 상승 전환 가능성을 설명할 수 있는 초기
변화가 여러 방향에서 나타나기 시작했지만, 최초의 유의미한 전환 `TRIGGER`라고
부르기에는 결정적 증거가 부족한 상태야.

하락 기울기 둔화, 저점 방어, 주봉 MA 평탄화 시작, Range 상단 접근, 주봉 고점
구조 개선 시도 등이 예가 될 수 있어. `SETUP`은 관찰 우선순위가 높아진 초기
준비 상태이지 매수 신호가 아니야.

핵심 경계는 `WATCH < SETUP < TRIGGER`야.

## `TRIGGER`

`TRIGGER`는 Pattern A FAST의 핵심 Stage야.

핵심 질문은 “주봉 구조가 단순 반등을 넘어 실제 상승 전환 가능성을 처음으로
의미 있게 보여주는가?”야.

주봉에서 상승 전환 `SETUP`이 충분히 성숙하고, 단순 기술적 반등이라고 보기 어려운
구조적 확인이 최초로 나타난 상태를 뜻해. Pattern A의 보수적 확인 시점보다 빠른
전환 가능성을 포착하는 것이 목적이지만, `TRIGGER` 자체가 무조건적인 매수 신호는
아니야.

월봉 국면, 일봉 타이밍, Investability, Flow, RS는 별도 축이야.
...
발전한다는 증거가 나타난 상태야. Pattern A의 `BASE`, `TRANSITION`, `EARLY_TREND`
등과 같은 시점에 나타날 수 있지만, 두 패턴의 Stage가 일치할 필요는 없어.

과거 snapshot에서 실제 `TRIGGER`를 관측한 것이 `TREND`의 필수 조건은 아니야.
`SETUP → TREND`나 `WATCH → TREND`처럼 `TRIGGER`를 건너뛴 direct jump도 정상일 수
있어. 이 경우 존재하지 않는 과거 `TRIGGER`나 Trigger Event를 추정하거나
backfill하지 않아.
...
핵심 질문은 “Fast 신규 진입 관점에서 움직임이 이미 상당 부분 진행됐는가?”야.

주봉 상승 구조 자체는 강하거나 정상일 수 있지만, Pattern A FAST가 목표로 하는
초기 전환·초기 진입 구간을 상당 부분 지난 상태를 뜻해.
...
있어. 다만 Fast 신규 진입 관점의 초기 Risk / Reward는 악화된 상태로 해석해.

`EXTENDED`는 terminal state가 아니야. 건강한 조정·횡보 뒤
`EXTENDED → TREND`가 가능하고, 구조가 충분히 reset되면
`EXTENDED → WATCH` 또는 `SETUP`도 개념상 가능해. 실제 Episode reset semantics는
별도 연구에서 다뤄.
...
`TRIGGER` 주야. 이어지는 `TRIGGER` 주를 새 Event로 중복 기록하지 않아.
...
이 경로는 `PRIMARY PATH`이지 강제 State Machine이 아니야.
...
Stage regression 자체를 Classifier Error로 간주하지 않아.
...
수 있지만, direct jump 자체를 금지하지 않아.

### Direct jump와 Trigger Event의 분리

`SETUP → TRIGGER → TREND`처럼 실제로 `TRIGGER` Stage를 거친 경우에만 Trigger
Event가 있어. `SETUP → TREND`처럼 `TRIGGER`를 건너뛴 episode에는 과거 Stage를
추정한 synthetic/inferred Trigger Event Date를 만들지 않아.

## PIT와 완료된 주봉 원칙

### 완료된 주봉만 사용

Weekly Lifecycle Stage는 기본적으로 완료된 주봉 데이터만 사용해. 특정 `as_of`
시점에 진행 중인 미완성 주봉을 완료된 주봉처럼 사용하지 않아. 주중 급등·급락으로
Stage가 일시적으로 왜곡되는 것을 막기 위해서야.
...
Monthly Regime은 별도 분류 축이지만 기본적으로 완료된 월봉을 사용해. 미완성 월봉을
확정된 Monthly Regime처럼 취급하지 않아. Current Month 정보를 사용해야 한다면
완료 월봉 기반 Regime과 섞지 않고 `Current Month Supporting Observation`으로
분리해.

### Point-in-Time 계약

Stage(t)는 t 시점까지 확정된 데이터만 사용해.

현재 Stage 판단에 다음을 사용하면 안 돼.
...
미래 데이터는 Ground Truth, Outcome Audit, Failure Analysis에서만 사용해.
...
유지해. `Fast Score >= 80 => TRIGGER`, `Fast Score >= 90 => TREND` 같은 구조는
허용하지 않아.

Stage는 주봉 생애주기의 구조적 의미를 표현하고, Score는 별도의 연속 측정값이야.
...
`FALSE_TRIGGER`는 Stage가 아니야. 미래 Outcome을 본 뒤 붙이는 Review Label이며,
당시 PIT Stage를 `FALSE_TRIGGER`로 rewrite하지 않아. 같은 이유로
`GOOD_TRIGGER`, `TOO_EARLY`, `TOO_LATE`도 Stage가 아니야.

Lifecycle Stage `EXTENDED`와 사후 Label 이름이 충돌하지 않도록 사후 Label은
`TOO_EXTENDED`를 사용해. Lifecycle Stage의 `EXTENDED` 이름은 유지해.

예를 들어 PIT 주봉이 `SETUP`이고 사후 리뷰가 `TOO_EARLY`일 수 있으며, PIT 주봉이
`TREND`이고 사후 리뷰가 `TOO_LATE`일 수도 있어.

### `UNAVAILABLE`과 `NOT_EVALUATED`

데이터 부족·손상·PIT weekly snapshot 계산 불가를 정상 Stage에 억지로 배정하지 않아.
특히 `WATCH`로 fallback하지 않아.

권장 상태는 `weekly_lifecycle_stage = UNAVAILABLE`이야. 다만 `UNAVAILABLE`은
`WATCH`·`SETUP`·`TRIGGER`·`TREND`·`EXTENDED`와 같은 lifecycle state가 아니라
Data / Evaluation Status야.

`NOT_EVALUATED`는 사용자가 분석을 요청하지 않았거나 필수 upstream 구조를 평가하지
않은 경우에 별도로 사용할 수 있어. Monthly Regime이 `BAD`라는 이유만으로 Weekly
Stage를 `NOT_EVALUATED`로 숨기지는 않아.

### 월봉·일봉과의 독립성

`Monthly: BAD`, `Weekly: TRIGGER`가 발생해도 Weekly Stage를 `WATCH`로 rewrite하지
않아. 월봉이 Fast Candidate를 허용하는지는 별도 해석 축이야.

`Weekly: TRIGGER`, `Daily: EXTENDED / WAIT`도 가능해. 일봉 타이밍이 좋지 않다고
주봉을 `SETUP`으로 낮추지 않아. 반대로 `Weekly: WATCH`에서 일봉이 READY처럼
보이는 단기 급등만으로 주봉을 `TRIGGER`로 올리지 않아.

### Investability·Flow·RS와의 독립성

다음은 Weekly Stage에 영향을 주지 않아.
...
조합이야. Weekly Stage는 가격 구조의 생애주기만 표현해.
...
`SETUP → TRIGGER`, `TRIGGER → TREND`야. 같은 Stage 반복은 Transition Event가
아니야.

### Re-Trigger와 Episode

한 종목에서 여러 Trigger Event가 발생할 수 있어.

예를 들어 `SETUP → TRIGGER → WATCH` 이후 `WATCH → SETUP → TRIGGER`가 되면
두 번째 `TRIGGER`는 새로운 Trigger Event 후보가 될 수 있어. 다만 새 Episode를
인정하는 조건이나 reset에 필요한 기간은 이 계약에서 수치로 정하지 않아.

여러 Event가 있을 때 first trigger only, best trigger, latest trigger 중 하나를
임의로 고르지 않아. 각 Trigger Event를 episode 단위로 관리할 가능성을 열어둬.

### Lead Time 처리

Pattern A 비교의 기준점은 Trigger Event Date야. 비교 대상은 Pattern A
`TRANSITION` first date와 `EARLY_TREND` first date야.

Pattern A가 해당 Stage에 도달하지 않은 종목은 Fast 실패로 자동 처리하지 않아.
실제 Trigger Event가 없는 direct jump episode의 Lead Time은
`NOT_EVALUATED` 또는 `NOT_APPLICABLE` 계열 상태로 처리해.
...
임의 생성하지 않아. 개념적으로 `trigger_event = NOT_OBSERVED`로 기록할 수 있지만,
정확한 production schema enum은 별도 Schema 계약에서 정해.

핵심 원칙은 하나야. **관측되지 않은 Trigger Event Date를 추정하거나 backfill하지
않는다.**

Human Ground Truth를 기록할 때도 PIT 구조 상태와 사후 Outcome Label을 분리해.
...
결론: Weekly Stage는 `TRIGGER`로 유지하고, 최종 후보 여부는 별도로 판단해.

### 일봉 충돌

Weekly: `TRIGGER`, Daily: `WAIT`
결론: Weekly Stage는 `TRIGGER`로 유지해.

### De-extension

`EXTENDED → TREND`
건강한 조정 뒤 다시 진행되는 개념적 사례야.
...
이 direct jump 자체는 Stage 오류가 아니야. 실제 발생 빈도는 별도 검증에서 감사해.

## 이 계약에서 정하지 않는 것

다음은 이 문서에서 새로 확정하지 않아.
...
의미를 동결한 기록이야. 다음 질문들은 이 문서에서 임의로 답하지 않은 역사적
연구 질문이며, 현재 적용 중인 Stage 의미를 뒤집지 않아.
