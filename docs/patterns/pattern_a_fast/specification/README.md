...
> Pattern A FAST는 Pattern A보다 빠른 상승 전환 구조를 탐지하는 **패턴**이야.
> `A FAST Core`는 이 패턴을 이용해 진입·보유·청산을 결정하는 **전략**이고,
> 둘은 같은 개념이 아니야.

## 핵심 요약

Pattern A FAST는 다음 질문에 답하기 위한 패턴이야.

> 장기 환경이 허용되는 종목에서, 상승 추세가 완전히 확인되기 전에 주봉 구조가
> 유효한 상승 전환으로 발전하기 시작했는가?

핵심 목적은 단순히 빨리 포착하는 것이 아니라 **빠르면서 구조적으로 유효한
(`EARLY + STRUCTURALLY VALID`) 전환**을 찾는 거야. 가격이 잠깐 올랐다는
사실만으로 좋은 신호나 `TRIGGER`로 판단하지 않아.

이 문서는 알고리즘, Feature 공식, Threshold, Score 산식을 정의하지 않아.
Pattern A FAST가 무엇을 탐지하는 패턴인지와 각 시간축·상태의 의미를 설명하는
개념 기준 문서야. 주봉 생애주기의 상세 계약은
[weekly_lifecycle.md](weekly_lifecycle.md)에서 확인해.

## Pattern A와의 관계

Pattern A FAST는 기존 Pattern A를 대체하거나 수정하지 않는 독립 패턴이야.
...
Pattern A를 정답 Label로 사용하지 않아. Pattern A가 나중에 신호를 냈는지만으로
Pattern A FAST의 성공 여부를 정하지 않는다. Pattern A는 Fast Trigger Date와
Pattern A `TRANSITION`·`EARLY_TREND` Date의 **Lead Time 비교 기준**이야.
Pattern A가 잡지 못한 상승 종목을 자동으로 Pattern A FAST 실패로 처리하지 않아.
실제 구조와 사후 Outcome은 별도로 검토해.
...
월봉은 “지금 빠른 상승 전환을 살펴볼 만한 장기 위치인가?”를 확인해.
월봉은 `TRIGGER`를 만들지 않고 환경·국면 필터 역할만 해.
...
주봉이 Pattern A FAST의 핵심 시간축이야. 주봉은 “장기 하락 또는 정체 상태에서
실제 상승 전환 구조가 만들어지기 시작했는가?”를 확인해.

구조를 설명할 때 사용하는 개념은 다음과 같아.
...
구체적인 계산식과 Threshold는 이 개념 문서에서 확정하지 않아. 현재 주봉 상태의
정의와 전이 원칙은 [주봉 생애주기 계약](weekly_lifecycle.md)이 담당해.

### 일봉: 진입 타이밍 보조

일봉은 패턴 자체를 결정하지 않아. 주봉 구조가 유효하다는 전제에서 현재 단기
진입 위치가 적절한지 보조해.

살펴볼 수 있는 개념은 돌파 직후, 눌림, 재돌파, 단기 이동평균 이격, 단기 과열,
최근 저점 이탈, 거래대금 동반 여부야. 일봉은
`ENTRY TIMING SUPPORT LAYER`이고, 일봉 상태 때문에 주봉 Stage를 임의로
낮추거나 높이지 않아.

## 목표 구간

Pattern A FAST가 찾고 싶은 구간은 다음과 같아.
...
주봉의 공식 생애주기는 `WATCH`, `SETUP`, `TRIGGER`, `TREND`, `EXTENDED`야.
각 상태의 상세 의미와 전이 규칙은 [weekly_lifecycle.md](weekly_lifecycle.md)에
고정되어 있어.

### `TOO_EARLY`

아직 구조적 증거 없이 가격만 단기적으로 움직인 상태야.
...
가격이 올랐다는 사실만으로 `TRIGGER`가 되지 않아.

### `SETUP`

`SETUP`은 아직 `TRIGGER`가 아니야. 상승 전환 가능성을 보여주는 주봉 구조
변화가 시작됐지만, 실제 전환을 확정할 정도의 구조가 완성되지 않은 상태야.

하락 기울기 둔화, 저점 방어, 주봉 MA 평탄화 시작, Range 상단 접근, 주봉 고점
구조 개선 시도 등이 예가 될 수 있어. `SETUP`은 관찰 대상이지 매수 신호가
아니야.

### `TRIGGER`

`TRIGGER`는 Pattern A FAST의 핵심 연구 대상이야. 월봉 환경이 허용 가능한
상태에서 주봉 가격 구조가 단순 반등을 넘어 상승 전환 가능성을 처음으로
의미 있게 보여주는 시점이야.

`TRIGGER`는 상승이 완전히 확인된 시점보다 빨라야 하지만 단순 반등보다 충분한
구조적 증거를 가져야 해. 개념적 목표는 다음과 같아.
...
뜻은 아니야. `TRIGGER`가 자동 매수 신호라는 뜻도 아니야.

### `TREND`

`TREND`는 Fast Trigger 이후 주봉 상승 구조가 지속되고 있는 상태야. Pattern A의
`TRANSITION` 또는 `EARLY_TREND`와 겹칠 수 있지만, Fast 관점에서는 최초 진입
Trigger가 아니라 진행 중인 추세로 봐.

### `EXTENDED`

`EXTENDED`는 초기 진입 구간을 상당 부분 지난 상태야. 나쁜 종목, 하락 예상,
매도 신호를 뜻하지 않아. Pattern A에서는 `EARLY_TREND`일 수도 있고 장기 투자
관점에서는 여전히 유효할 수 있어. 다만 Fast 신규 진입 관점에서는 Risk / Reward가
악화된 상태로 해석해.

`EXTENDED`는 terminal state가 아니야. 건강한 조정이나 횡보 뒤
`EXTENDED → TREND`가 가능하고, 구조가 충분히 reset되면
`EXTENDED → WATCH` 또는 `SETUP`도 개념상 가능해. 실제 Episode reset 규칙은
별도 계약에서 다뤄.

### `FALSE_TRIGGER`

`FALSE_TRIGGER`는 `TRIGGER` 직후 기존 하락 구조로 복귀하는 경우를 뜻하는
사후 평가 Label이야. 주봉 돌파 실패, 주봉 저점 구조 붕괴, 장기 Range 하단
회귀, 지속적인 상승 구조로 연결되지 못한 반등 등이 예야.

`FALSE_TRIGGER` 판정 기간은 이 정의에서 공식 Threshold로 고정하지 않아.

### `TOO_LATE`

`TOO_LATE`는 주봉 전환이 이미 충분히 진행된 뒤 뒤늦게 Fast Trigger가 발생한
경우야. Fast의 가치가 낮아지는 사례를 표현하지만, 개별 사례 하나만으로 실패로
판정하지 않고 전체 코호트에서 평가해.

## 성공 신호의 의미

`SUCCESSFUL FAST SIGNAL`은 다음 조건을 함께 만족하는 신호야.

- Trigger 이후 주봉 상승 구조가 의미 있게 유지되거나 발전함
- 기존 장기 하락·정체 구조로 즉시 복귀하지 않음
- Pattern A보다 유의미한 선행 탐지 가능성을 제공함

성공을 단순 수익률로 정의하지 않아. 예를 들어 Trigger 후 `+10%` 같은 단일
...
어떤 수치가 PASS 기준인지 이 문서에서 정하지 않아. 먼저 실제 Ground Truth의
분포를 확인한 뒤 별도 검증에서 판단해.

## Ground Truth Label

주봉 Lifecycle Stage와 사후 Ground Truth / Outcome Label은 분리해.

- **Lifecycle Stage**: PIT 시점의 주봉 구조 상태
- **Ground Truth Label**: 이후 구조를 사람이 검토해 붙이는 사후 결과 Label

사람이 사용할 수 있는 Label의 의미는 다음과 같아.
...
특정 historical week의 판단에는 그 주 시점까지 확정된 데이터만 사용해.

Trigger 계산에 미래 주가, 미래 거래량, 미래 Stage, 미래 Pattern A 판정, 미래
수익률을 사용하지 않아. 미래 데이터는 사후 Validation, Ground Truth, Outcome
Audit와 Failure Analysis에서만 사용해.

## 다른 분석 축과의 분리

Pattern Detection은 다음 축과 섞지 않아.
...
Pattern A FAST는 `RS` 데이터가 없어도 계산 가능한 독립 패턴이어야 해. 예를 들어
`Pattern A Fast TRIGGER`이면서 동시에 `FILTERED_MARKET_CAP`일 수 있고, 이것은
정상적인 결과야. `A FAST Core` 전략이 이 결과를 이용할지는 별도 전략 계약에서
결정해.

## 이 문서의 범위 밖인 것

이 정의는 다음을 공식 규칙으로 만들지 않아.
