...
진입 시점과 진입 가격은 V2와 동일하다. 후속 matched A/B 청산 검증에서도 CONTROL 진입 코호트는 고정하며, 후보별 독립 진입 차이를 만들지 않는다.
...
- 동일 종목 동시 보유는 금지한다.
- 청산 후 상태는 완전히 리셋한다.
- 후속 청산 규칙 matched A/B 검증에서는 CONTROL 진입 코호트를 고정하며 후보별 독립 재진입을 생성하지 않는다.
...
- V4는 아직 공식 전략이 아니다.
- V2가 계속 현재 기본 전략이다.
- 다음 단계는 전략 생애주기 4단계인 검증 계획 확정이다.
