# A FAST Core V1 — 역사적 기준 전략

> V1은 현재 기본 전략이 아니라, V2와 비교할 때 사용하는 역사적 동결 기준선이야.

## 현재 상태

- **전략 ID**: `PATTERN_A_FAST_FINAL_STRATEGY_V01`
- **전략 상태**: 동결 (`FINAL_STRATEGY_FROZEN`)
- **연구 상태**: 전략 확정 완료 (`STRATEGY_FINALIZATION_CLOSED`)
- **연구 분류**: `STRATEGY_FINALIZATION_FROZEN_CONTRACT`
- **연구 출처**: `SAME_SAMPLE_RETROSPECTIVE_FINALIZATION`
...
- **사전등록 근거 커밋**: [`a5c29e7`](https://github.com/RozeKurhy/krx-trend-scanner/commit/a5c29e7e97cb7e6830c3dcd25d824e5779f2312f)
...
- **최종 선택 권한**: `FINAL_STRATEGY_CONTRACT` (`docs/patterns/pattern_a_fast/strategy/version_01/README.md` + `artifacts/patterns/pattern_a_fast/production/strategy_v01/pattern_a_fast_final_strategy_v01.json`)
- **계약 권한**: `THIS_FINAL_STRATEGY_CONTRACT_REVISION`
- **Fresh OOS 상태**: `READY_FOR_PREREGISTRATION` (실행 여부: `NO`)
- **운영 상태**: 운영 보류 (`PRODUCTION_HOLD`)

## 투자 원칙

- **최우선 목적**: 대형 손실 최소화 (`LARGE_LOSS_MINIMIZATION`)
- **차순위 목적**: 충분한 상승 수익 보존 (`PRESERVE_SUFFICIENT_UPSIDE`)
- **핵심 철학**:
...
### 3) NEVER_PROGRESSED와 미청산 거래
- 평가 종료일(Cutoff)까지 미청산된 거래는 **`OPEN_AT_CUTOFF`**로 유지 (`forced_close: false`, `valuation: MARK_TO_CUTOFF`).

## 알려진 위험과 한계
1. **대형 손실 방어 vs 승률/기대수익 절단 비용**:
   - Pre-PROGRESSED Loss Guard는 <= -30% 극단 손실(6건, 1.09%) 및 <= -20% 손실(25건, 4.54%)을 효과적으로 억제하여 파산 위험을 획기적으로 낮춘다.
   - 반면 약 53.4%의 높은 손절 발동률(294건)로 인해 승률(39.93%)과 중앙값 수익률(-13.60%)이 감소하며, Loss Guard가 없었다면 E1 기준 terminal return이 +50% 이상이었을 63건의 대형 승자(및 +100% 이상 32건)가 조기 마감되는 명확한 기회비용이 존재한다.
2. **손절 거래의 Counterfactual MFE 특성**:
   - 손절된 294건 거래의 잠재 최대 상승률(Counterfactual MFE)은 평균 79.60%, 중앙값 37.79%로 나타남.
3. **익일 시가 체결 갭 위험 (Execution Gap Risk)**:
   - 손실 가드는 일봉 종가 -15% 도달 시 발생하여 익영업일 시가에 체결되므로, 갭 하락 시 실제 실현 손실이 -15%를 초과할 수 있다 (표본 내 최악 손실률 -49.40%, 최악 MAE -59.27%).
