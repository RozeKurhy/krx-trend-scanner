...
가격, 데이터 범위, 캘린더 의미와 기준일은 V2와 V3에서 동일하게 유지한다.
...
종목 수가 542개이다. 당시 계획에서는 실행 시 실제 파일과 값이 다르면 성과
비교를 진행하지 않고 검증을 중단하여 보고하도록 했다.

### 동일 진입 보장

CONTROL의 **973개 진입을 모두 그대로 사용**한다. 각 거래에서 다음 항목은 V2와
V3 재현 과정 사이에 완전히 동일해야 한다.
...
각 CONTROL 진입을 V2 청산 규칙과 V3 청산 규칙 아래 독립적으로 재현한다. 한
CONTROL 거래의 재현 과정이 다른 CONTROL 진입을 삭제하거나 이동해서는 안 된다.
...
- 외부 API, 네트워크 요청, 캐시 갱신과 누락 데이터 자동 다운로드를 사용하지
  않는다.
- 식별자 생애주기와 Point-in-Time(PIT) 원칙을 유지한다.
- 완료된 정보만 사용한다.
- 미래 주가, 미래 거래량, 미래 Stage, 미래 성공 여부를 현재 판단에 사용하지
  않는다.
- 데이터 누락·손상·식별자 불일치는 실패 시 종료(`fail-closed`)로 처리하고 검증 실패 사유를
  기록한다.
...
Pre-Winner 손절, `W25`, `W30`, `FAILURE_ARMED`, 지속성 청산, 새로운
MFE/HWM 임계값, 임의의 Soft/Hard 보정을 추가하거나 조정하지 않는다.
...
V2와 V3 각각에 대해 평균·중앙값 최종 수익률(terminal return)과 승률을 계산한다.
또한 다음 동일 거래 비교 지표를 계산한다.

- V3 - V2 동일 거래 수익률 차이의 평균
- V3 - V2 동일 거래 수익률 차이의 중앙값
- `improved` 거래 수
- `worsened` 거래 수
- `same` 거래 수

동일 거래 비교 결과를 단순 총합 수익률보다 우선한다.
...
V2와 V3 각각에 대해 평균·중앙값 MAE와 다음 최종 수익률 손실 구간을 비교한다.

- `<= -15%`
- `<= -20%`
- `<= -30%`
- `<= -40%`
- 기존 데이터가 제공하는 경우 더 깊은 손실 구간
...
V2와 V3 각각에 대해 최종 수익률 `>= +20%`, `+30%`, `+50%`, `+100%`,
`+200%`, `+400%` 구간을 비교한다. MFE 대비 최종 수익률 되돌림도
계산하며, V2가 조기 종료한 대형 잠재 승자를 V3가 얼마나 보존하는지 확인한다.

### E. V3 Pre-Winner 위험

각 고정 진입에 대해 전략 청산과 무관한 공통 전체 경로 raw MFE를 사후 진단용으로
계산한다.
...
Pre-Winner 코호트에서 최종 수익률(terminal return), MAE, 보유 기간,
...
각 이유에 대해 거래 수, 비율, 최종 수익률, MAE, MFE, 보유 기간과 대표
실패 사례를 기록한다.

## 6. MDD 처리

이번 검증은 973개 고정 진입을 각각 독립 재현하는 청산 규칙 동일 진입 A/B이다.
동일한 포지션 사이징과 포트폴리오 구성 규칙이 기존에 확정되어 있지 않다면
새로운 포트폴리오 가정을 만들지 않는다.

기본 하방 위험 평가는 거래 단위 MAE, 최종 손실 구간, 보유 기간과
`OPEN_AT_CUTOFF`를 우선한다. V2와 V3에 동일하게 적용 가능한 확정 포트폴리오
자산가치 곡선이 이미 존재하는 경우에만 MDD를 보조 지표로 사용할 수 있다.
...
Soft/Hard 임계값의 여러 조합을 돌리는 민감도 탐색은 수행하지 않는다.
...
통계적 유의성 검정과 별도 최소 개선폭 임계값은 이번 후보 검증에서 새로
만들지 않는다.
...
3. `<= -30%` 손실 비율이 V2 이하
4. `<= -40%` 손실 비율이 V2 이하
5. 대형 Winner 보존 또는 MFE 대비 되돌림이 V2보다 개선
6. 중앙값 보유 기간이 V2 이하
7. `OPEN_AT_CUTOFF` 비율이 V2 이하
8. 다음 강건성 분해 조건을 모두 만족
   - 시장 분해: KOSPI의 평균 동일 거래 수익률 차이 `>= 0` 및 KOSDAQ의 평균 동일
     거래 수익률 차이 `>= 0`
   - 진입 유형 분해: 최초 진입의 평균 동일 거래 수익률 차이 `>= 0` 및 재진입의
     평균 동일 거래 수익률 차이 `>= 0`
   - 진입 연도 분해: 거래가 존재하는 각 진입 연도의 평균 동일 거래 수익률 차이를
     계산하고, 그중 절반 이상에서 평균 동일 거래 수익률 차이 `>= 0`
...
코호트, 실행 의미와 동일 진입 재현의 데이터 구조 확인에만 사용한다.
...
기준 변경, 탐색 결과를 동일 진입 A/B 결과로 간주하는 행위, 기존 V3 독립 진입
...
- V2 또는 V3 운영 전략 코드와 운영 파이프라인을 수정하지 않는다.
- Stock Report, Candidate, Ranking을 수정하지 않는다.
- 계획 확정 작업에서 산출물을 생성하거나 수정하지 않는다.
- 계획 문서가 결과 생성 전에 커밋되어 있어야 한다.
- CONTROL 거래 수가 973건이고 고유 종목 수가 542개여야 한다.
- 기간은 `2021-04-01`부터 `2026-08-21 CLOSE`까지, 신호 기준일은
  `2026-08-14`로 고정한다.
- V2와 V3가 동일 CONTROL 진입을 사용하고 진입 날짜·시가 100% 일치 검사를
  수행해야 한다.
- Pre-Winner `+20%` 미도달 코호트, 손실 구간, Winner 보존·수익 되돌림, 보유 기간과
  `OPEN_AT_CUTOFF`를 집계해야 한다.
- 파라미터 탐색과 임계값 민감도 탐색을 수행하지 않는다.
