...
이 문서는 동결 후보 `A FAST Core V4`와 현재 기본 전략 `A FAST Core V2`를 동일한 진입 코호트에서 비교하기 위한 공식 검증 계획이다. 백테스트를 실행하기 전에 비교 대상, 고정 진입, 데이터 범위, 평가 지표, 실패 수리 기준, 공식 전략 채택 기준과 기본 전략 승격 기준을 사전 확정한다.

이번 문서는 전략 생애주기의 **4단계: 검증 계획 확정**에 해당한다. 이 문서가 확정된 뒤 별도 승인된 작업에서 동일 조건 비교 백테스트를 실행한다. 본 작업에서는 백테스트, 코드 구현, 테스트 실행, 결과 생성과 artifact 생성을 수행하지 않는다.

결과를 확인한 뒤 합격 기준이나 비교 조건을 변경하지 않는다. 규칙 변경이 필요하면 V4를 수정하지 않고 새 후보 전략으로 다시 정의·동결한다.
...
이번 비교에서 달라지는 요소는 청산 규칙뿐이다. 진입 규칙, 진입 시점, 진입 가격, 데이터 범위, 캘린더 의미와 cutoff는 V2와 V4에서 동일하게 유지한다.
...
하나라도 만족하지 않으면 자동 공식 채택 자격 없음으로 기록한다. 최종 채택 결정 자체는 생애주기 후속 단계에서 수행한다.
...
이 문서가 리뷰를 통과하면 다음 단계는 전략 생애주기 5단계인 **동일 조건 비교 백테스트**다. 다음 단계에서도 결과를 보기 전에 고정한 규칙과 기준을 변경하지 않는다.
