...
> 이 문서는 `A FAST Core V3` 후보 전략 검증을 위해 작성했던 과거 A/B 검증
> 계획을 보존하는 역사 기록이다. 현재 진행 중인 검증이나 향후 확정 계획을
> 의미하지 않는다.

당시에는 `A FAST Core V3`와 현재 기본 전략 `A FAST Core V2`를 동일한 진입
코호트에서 비교하기 위해 청산 규칙, 데이터 범위, 평가 지표, 실패 사례, 공식
전략 채택 기준과 기본 전략 승격 기준을 고정했다. 당시 문서는 전략 생애주기의
**4단계: 검증 계획 확정**에 해당했으며, 계획 작성 작업에서는 백테스트, A/B
결과 생성, 코드 수정, 테스트 실행, 아티팩트 생성을 수행하지 않았다.
...
공식 검증 결과는 당시 이 계획이 커밋된 이후 동일 조건으로 새로 생성하도록
기록했다. 현재 이 문서는 실행 지시가 아니라 과거 계획 기록이다.

## 13. 당시 불변 원칙과 실행 전 체크리스트
