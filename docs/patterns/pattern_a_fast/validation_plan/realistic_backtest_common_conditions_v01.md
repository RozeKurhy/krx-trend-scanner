...
기준일 대체 없이 `fail-closed`한다. 결측 OHLC·시장 데이터·메타데이터와
...
체결가·평가가격을 확보할 수 없는 경우 기준일 대체 또는 종가 대체를
...
현재·미래 시가총액, 기준일 대체, 임의 대체값(proxy)을 사용하지 않고
...
없으면 기준일 대체 없이 실행 계약의 실패 시 종료(`fail-closed`) 또는 검토 상태를
따른다.
...
Sequential의 `cutoff_valuation_price`와 일치하지 않으면 기준일 대체나
