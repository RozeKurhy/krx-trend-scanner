...
현실적 백테스트에서는 아래 항목을 숫자로 최종 고정해야 한다. 현재 레포에서
이 항목들을 FastCore realistic 공통조건으로 승인한 계약은 확인되지 않았다.
...
문서의 상태를 `확정`으로 바꾸지 않으며, 결과 계산도 실행하지 않는다.
...
- 매도 현금 유입: 체결 notional에서 매도 수수료와 세금 차감
...
- **exposure**: 일별 invested market value / 일별 equity; 평균·최대값을
  구분하여 보고
- **turnover**: 실행된 매수·매도 notional의 절대합을 초기 자본과 비교해
  보고하며, 비용 포함 여부와 정규화 기준은 최종 계약에서 고정
...
1. 매수·매도 수수료, 매도 세금, 매수·매도 슬리피지의 승인 숫자와 적용 계약
...
5. 전략 중립 benchmark와 동일기간 비교 방식
6. 공통 market regime 분류 기준 및 결과 분할 방식
7. strategy-specific parameter robustness의 범위와 보고 형식
