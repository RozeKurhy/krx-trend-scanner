...
| 4. 검증 계획 확정 | 미완료 | [Julia 공식 전략 검증 계획 V01](validation_plan_v01.md) 초안 작성, 검토 및 동결 대기 |
...
현재 다음 공식 단계는 4단계 검증 계획 검토 및 동결이다. 계획 초안은
`검토 대기` 상태이며, 검토·동결 전에는 검증을 시작하지 않는다.
...
- [Julia 공식 전략 검증 계획 V01](validation_plan_v01.md): 공식 검증 전
  비교 조건·데이터·기간·지표·판정 기준을 사전 고정하는 검토 대기 초안이다.
