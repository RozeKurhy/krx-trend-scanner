...
**알려진 한계 3 — 해결됨 (`PyKrxDataProvider`에서 1원 이내만 보정)**: phantom row를
제거해도 NAVER(035420) 15건, 삼성전자(005930) 1건의 OHLC 관계 위반이 남았습니다.
phantom row와는 다른 패턴으로, 값 자체는 정상적인 거래일 데이터인데 `high`가
`open`/`close`보다 정확히 1원 낮은 식의 사소한 반올림 오차입니다(예: 005930
2016-10-24 open=31860 고가=32159 종가=32160 → high가 close보다 1 작음). 두 종목
모두 이후 액면분할(NAVER 2018-10, 삼성전자 2018-05)이 있었던 종목이라, 수정주가
계산 시 분할 이전 구간의 각 필드(시가/고가/저가/종가)를 독립적으로 반올림하면서
생기는 오차로 추정됩니다(확정하지 않음, 관찰만).

Validator를 느슨하게 만들지 않고, `_correct_minor_rounding_violations`로
`adjusted=True` 경로에서만 다음 정책으로 보정합니다.

- `high`가 `open`/`close`보다 낮고, 정상값(`max(open, close)`)과의 차이가 1원
  이하이면 `high`를 정상값으로 보정한다.
- `low`가 `open`/`close`보다 높고, 정상값(`min(open, close)`)과의 차이가 1원
  이하이면 `low`를 정상값으로 보정한다.
- 2원 이상 벌어진 위반은 보정하지 않고 `validate_ohlcv`가 그대로 `MarketDataError`로
  실패시킨다.
- `adjusted=False`(원본) 경로는 이 보정을 적용하지 않는다.

실측(2016-08-14~2026-08-14, 10년): NAVER 15건(2016-08-18, 2016-12-16, 2017-02-10,
2017-02-16, 2017-04-06, 2017-04-21, 2017-04-25, 2017-05-04, 2017-10-27, 2017-12-15,
2018-01-22, 2018-03-08, 2018-04-09, 2018-08-01, 2018-08-03), 삼성전자 1건
(2016-10-24) 모두 `high`가 정상값보다 정확히 1원 낮은 경우였고, 전부 보정 후
`validate_ohlcv`를 통과했습니다. 068270/000660은 이런 위반이 없어 보정 대상이
0건입니다.
